    ...
    Citizen.CreateThread(function()
        ...
        if IsPedInAnyVehicle(playerPed) then
            ...
            if tostring(string.upper(plate)) == tostring(string.upper(immatricul)) then	
                ...
                TriggerServerEvent('garages:SetVehIn', immatricul)
                TriggerServerEvent("GTA-Coffre:resetItem", immatricul)
                TriggerEvent("NUI-Notification", {"Véhicule rentré !"})
            else
                if getEmplacement <= max then
                    ...
                    TriggerServerEvent('garages:SetVehicule', "Mon vehicule", model, immatricul, primarycolor, secondarycolor, pearlescentcolor, wheelcolor, zoneGarage)
                    Wait(150)
                    TriggerServerEvent('garages:CheckDuplicationVeh', zoneGarage, immatricul)
                    TriggerEvent("NUI-Notification", {"Véhicule rentré !"})
                else
